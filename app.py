"""
AI-stock-bot — Streamlit V2.1
登入 / 個股分析 / 觀察名單 / 目標價 / 帳號 / 管理員
資料庫：GitHub Gist  |  AI 報告：Groq（免費）
"""
...
with st.sidebar:
    ...
    st.caption(f"{'👑 管理員' if is_admin else '👤 一般用戶'}  |  @{user['username']}")
    st.markdown("---")
    ...
    if st.button("💾 儲存 Chat ID", use_container_width=True):
        ...
        st.success("✅ 已儲存")
    st.markdown("---")
    st.caption("SOP：KD金叉/多頭 + MACD翻紅 + SAR多方")
...
if is_admin:
    _tabs.append("👑 用戶管理")
tabs      = st.tabs(_tabs)
tab_ana   = tabs[0]; tab_wl  = tabs[1]
tab_tgt   = tabs[2]; tab_acc = tabs[3]
...
def sop_check(df):
    ...
    signal = None
    if hard_pass:
        signal = "SELL" if wlabel in ("3-5","B-c","C-3") else "BUY"
...
def fibonacci(df):
    ...
    return {"high":hi,"low":lo,
            "0.236":hi-d*0.236,"0.382":hi-d*0.382,
...
def push_tg(token, chat_id, signal, name, code, price, sop, buy_agg, buy_con, stop):
    ...
    pl = (f"🦁 激進:<b>{buy_agg:.2f}</b> 🐢 保守:<b>{buy_con:.2f}</b> 🛑 停損:<b>{stop:.2f}</b>"
          if signal == "BUY"
          else f"⚡ 出場:<b>{price:.2f}</b> 🛑 停損:<b>{stop:.2f}</b>")
...
def wchip(lbl):
    c = "wbull" if lbl.startswith(("3","4")) else \
        "wbear" if lbl.startswith(("C","B")) else "wneut"
    return f"<span class='wchip {c}'>{lbl}</span>"

def _v(row, key, fb=np.nan):
    v = row.get(key, fb)
    return fb if pd.isna(v) else v
...
with tab_ana:
    ...
    if run_btn:
        ...
        if signal in ("BUY", "SELL"):
            ...
            with tg_col:
                if st.button("📲 推播到Telegram", type="primary", use_container_width=True):
                    ok = push_tg(tg_token, tg_chat, signal, nm, cc,
                                 t["Close"], sop, buy_agg, buy_con, stop)
                    st.success("✅ 推播成功！") if ok else st.warning("⚠️ 請先填入Token/ID")
        else:
            hc   = sum([sop["cond_kd"], sop["cond_macd"], sop["cond_sar"]])
            rows = "".join([
                f"<div class='cond-row'><span class='{'pass' if ok else 'fail'}'>{'✅' if ok else '❌'} {lb}</span></div>"
                for ok, lb in [(sop["cond_kd"],   f"KD:{sop['kd_label']}"),
                               (sop["cond_macd"],  f"MACD:{sop['macd_label']}"),
                               (sop["cond_sar"],   f"SAR:{sop['sar_label']}")]
            ])
            soft_h = (f"<span class='hint'>💡 量比{vol_r}x≥1.5 ✔</span><br>"
                      if sop["cond_vol"]
                      else f"<span style='color:#888'>💡 量比{vol_r}x（未達1.5）</span><br>")
            soft_h += (f"<span class='hint'>{sop['wave_hint']} ✔</span>"
                       if sop["wave_hint"]
        ...
        with bc:
            ...
            st.metric("位置", bm)
            st.progress(float(np.clip(bb, 0, 1)))
        ...
        for col, mult, lb in [(t1,1.05,"短線+5%"),(t2,1.10,"波段+10%"),(t3,1.20,"長線+20%")]:
            tp   = t["Close"] * mult
            days = max(5, int((tp - t["Close"]) / max(atr*0.4, 0.01) * 2.5))
            col.metric(lb, f"{tp:.2f}", f"約{days}天")
        st.caption(f"更新：{datetime.now().strftime('%Y-%m-%d %H:%M')} | AI Stock Bot V2.1")

        # ── AI 深度報告書 ──
        st.markdown("---")
        st.markdown("## 🤖 AI 深度個股分析報告書")
        st.caption("整合技術面、基本面、波浪理論、美股連動、最新新聞（由 Groq Llama 3.3 免費生成）")

        if not AI_READY:
            st.warning("⚠️ `ai_report.py` 尚未上傳到 GitHub，請先上傳該檔案。")
        else:
            groq_key = ""
            try:
                ...
                pass
            if not groq_key:
                st.warning("⚠️ 請在 Streamlit Secrets 加入 `GROQ_API_KEY`（至 groq.com 免費取得）")
            else:
                if st.button("📋 生成完整分析報告書", type="primary",
                    ...
                    with st.spinner("🤖 AI 正在分析數據，生成完整報告書...（約 30-60 秒）"):
                    ...
                    if result.get("error"):
                        ...
                        if news_list:
                            ...
                            for n_item in news_list[:5]:
                                ...
                                ca.markdown(n_item["sentiment"])
                                cb.markdown(f"[{n_item['title']}]({n_item['link']}) `{n_item['pub']}`")
                        # 美股
                        if us:
                            st.markdown("#### 🌏 美股即時")
...
with tab_wl:
    ...
    if ab and nc:
        ...
        st.success(f"✅ 已新增 {nm_in}（{code_in}）")
        st.rerun()
...
with tab_tgt:
    ...
    with st.container(border=True):
        ...
        with t2c: tc_price = st.number_input("目標價", min_value=0.01, step=0.5, format="%.2f", key="tp", label_visibility="collapsed")
        ...
        for code, entry in my_tgts.items():
            ...
            try:
                for sfx in [".TW", ".TWO"]:
                    ...
                    if not df_tmp.empty:
                        price_now = float(df_tmp["Close"].iloc[-1])
                        break
            except:
                pass
            with st.container(border=True):
    ...
    if is_admin:
        ...
        if not all_tgts:
            ...
            for code, entries in all_tgts.items():
                nm_t = fetch_name(code)
                with st.expander(f"📌 {nm_t}（{code}）— {len(entries)} 人設定"):
                    for e in entries:
...
with tab_acc:
    ...
    with st.container(border=True):
        ...
        new_id = st.text_input("我的 Telegram Chat ID", value=cur_id,
                               placeholder="從 @userinfobot 取得")
